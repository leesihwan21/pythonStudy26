# 오류 회피하기 : 코드를 작성하다 보면 특정 오류가 발생해도
# 그냥 통과시켜야 할 때가 있다.
...
# 리스트에서 한 개씩 꺼내와 학생 변수에 넣음. student
    try:
        with open(f"{student}_성적.txt","r", encoding="utf-8") as f:
            score = f.read() # r은 파일 불러오기. 파일에 있는 내용 및 데이터를 불러와 score에 넣음.
            print(f"{student}의 성적 : {score}")
            # score 변수에 있는 내용을 출력한다는 문장.

    except FileNotFoundError:
        print(f"{student}의 성적 파일이 없습니다.")
        print("다음 학생으로 넘어갑니다.")
        continue # 다음 for 문 실행하기 위한 것.
