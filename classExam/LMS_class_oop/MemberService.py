...
9. 종료
    ...
    def member_login(self):
        ...
        member = self.find_member(uid) # 아이디를 받아서 객체에 전달
        ...
        if member.pw  == pw:
            self.session = member # index가 아니라 member 자체가 들어감. member 전체 주소가 들어감.
    ...
    def member_admin(self):
        ...
        while subrun:
            ...
            # 회원 목록 보기
            if sel == "1":
                self.show_member_list() # 회원의 목록을 봐야지 찾아서 사용할 수 있어서. 관리자 메뉴이니까.
            ...
            # 권한 변경
                ...
                uid = input("대상 아이디:")
                member = self.find_member(uid) # find_member 자주 사용함.
                if member:
    ...
    def show_member_list(self):
        ...
        for member in self.members:
        # members 리스트에 있는 객체를 하나씩 가져와 member에 집어넣음.
            status = "활성" if member.active else "비활성"
            #  member.active == True 이면, status 변수에 "활성"이라고 하고, 아니면 "비활성"
